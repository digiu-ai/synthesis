...
contract Synthesis is RelayRecipient {
    ...
    function emergencyUnsyntesizeRequest(bytes32 _txID, address _receiveSide, address _oppositeBridge, uint _chainID) whenNotPaused external {
    ...
    function burnSyntheticToken(address _stoken, uint256 _amount, address _chain2address, address _receiveSide, address _oppositeBridge, uint _chainID) external whenNotPaused returns (bytes32 txID) {
    ...
    // utils
    function setRepresentation(address _rtoken, address _stoken, uint _chainID) internal {
        representationSynt[keccak256(abi.encodePacked(_rtoken, _chainID))] = _stoken;
        representationReal[_stoken] = _rtoken;
    }

    function createRepresentation(address _rtoken, uint _chainID, string memory _stokenName, string memory _stokenSymbol) onlyOwner external {
        ...
        setRepresentation(_rtoken, address(syntToken), _chainID);
    }
